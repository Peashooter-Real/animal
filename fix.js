...
// Write alert implementations block
const alertCode = `
...
if (!code.includes('vgConfirm = function')) {
...
code = code.replace(/setTimeout\(\(\) => \{(\s+)if \(await vgConfirm/g, 'setTimeout(async () => {$1if (await vgConfirm');

// Jamil doesn't have "await vgConfirm" directly after bracket, let's catch it manually:
// From ride checking
code = code.replace(/setTimeout\(\(\) => \{(\s+)const vg = document\.querySelector/g, 'setTimeout(async () => {$1const vg = document.querySelector');
