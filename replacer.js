...
code = code.replace(/setTimeout\(\(\) => \{(\s+)if \(await vgConfirm/g, 'setTimeout(async () => { if (await vgConfirm');

// Jamil doesn't have "await vgConfirm" directly after bracket, let's catch it manually:
// From ride checking
code = code.replace(/setTimeout\(\(\) => \{(\s+)const vg = document\.querySelector/g, 'setTimeout(async () => { const vg = document.querySelector');
